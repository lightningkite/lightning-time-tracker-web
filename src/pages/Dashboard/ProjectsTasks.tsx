    ...
    if (currentUser.defaultFilters.states.length === 0) {
      ...
      stateCondition = {
        And: [
          ...
          {state: {NotInside: [TaskState.Cancelled, TaskState.Delivered]}}
        ]
      }
    }

    let projectCondition: Condition<AnnotatedTask>
    let userCondition: Condition<AnnotatedTask>
    if (currentUser.defaultFilters.projects.length === 0) {
      projectCondition = {Always: true}
      userCondition = {user: {Equal: currentUser._id}}
    } else {
      userCondition = {Always: true}
      projectCondition = {
              ...
              And: [
                {user: {Equal: currentUser._id}},
                {
                  state: {
                    Inside: [
                      TaskState.Active,
                      ...currentUser.defaultFilters.states
                    ]
                  }
                },
                {state: {NotInside: [TaskState.Cancelled, TaskState.Delivered]}}
